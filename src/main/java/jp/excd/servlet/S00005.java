...
public class S00005 extends HttpServlet {
		...
		try {
			...
			if ("1".equals(rating_radio)) {
				if (rating_from == null || "".equals(rating_from)) {
					// 処理継続
				} else if (Judge.isNumber(rating_from) == false || Integer.parseInt(rating_from) < 0 ) {
					// エラー
			...
			if ("1".equals(rating_radio)) {
				...
				} else if (Judge.isNumber(rating_to) == false || Integer.parseInt(rating_to) < 0 ) {
					//エラー
			...
			if ("1".equals(views_radio)) {
				...
				} else if (Judge.isNumber(views_from) == false || Integer.parseInt(views_from) < 0 ) {
					//エラー
			...
			if ("1".equals(views_radio)) {
				...
				} else if (Judge.isNumber(views_to) == false || Integer.parseInt(views_to) < 0 ) {
					//エラー
	...
	private List<SongRecord> executeQuery (HttpServletRequest request,
		...
		try {
			...
			} else if ("5".equals(sort_order)) {
				SQL += sql15;					
	
			} else if ("6".equals(sort_order)) {
				SQL += sql16;
	
			} else if ("7".equals(sort_order)) {
				SQL += sql17;
	
			} else if ("8".equals(sort_order)) {
				SQL += sql18;
			}
		} catch (Exception e) {
			throw new Exception();
